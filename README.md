...
Unraid GPU 虚拟化管理插件，在一个设置页面管理 **NVIDIA vGPU** 和 **Intel i915 SR-IOV**。提供**简体中文 / English** 界面、驱动安装与更新、vGPU 设备与虚拟机绑定，以及 **Unraid 升级前的驱动预下载**。
...
打开 **设置 → Unraid vGPU Manager**。页面右上角可选择简体中文、English 或跟随 Unraid，语言偏好保存在启动 U 盘。

| 标签页 | 内容 |
|---|---|
| 驱动管理 / Drivers | 驱动状态、安装 / 更新 / 卸载、Unraid 升级前准备、定时检查设置 |
| NVIDIA 显卡 / NVIDIA GPU | 驱动系列、授权、模块、unlock、vGPU 配置档、设备管理、虚拟机绑定 |
| Intel i915 SR-IOV | VF 数量、直通状态、启动参数 |

**首次安装插件不会自动安装 GPU 驱动。** 请在“驱动管理”中按需安装 NVIDIA 或 Intel 驱动；页面的“已启用”表示用户已经选择安装并由插件管理该驱动。卸载某个驱动会取消它的自动恢复和预下载。

## Unraid 升级前的驱动准备

Unraid 更新写入启动盘后，插件读取 `/boot/bzimage` 中**实际准备启动的内核版本**，与当前 `uname -r` 比较。系统更新完成钩子会立即检查，另有每分钟一次的定时检查覆盖其他更新方式。结果通过 Unraid 通知和插件页面显示。
...
- 默认开启“系统更新后自动准备已启用的驱动”，可在常规设置中关闭。此选项与“每天检查驱动更新”相互独立。
...
也可以在更新系统之前，从发行说明获得内核版本后，在“目标内核”填写完整名称，例如 `6.18.47-Unraid`，手动预下载。留空会读取启动镜像。插件不根据 changelog 中的历史条目猜测内核版本。
...
6. 更新或切换驱动系列前，停止使用 GPU 的 VM 和容器。修改“下次安装 / 更新使用的驱动系列”后，驱动管理页的更新按钮会使用该系列。切换宿主驱动分支时，还需核对 guest 驱动兼容性。
...
检查依赖：PHP CLI + SimpleXML、Python 3.9+、Node.js、ShellCheck、jq、bubblewrap。回归测试在无网络、无宿主 GPU 的隔离文件系统中运行，不调用宿主的驱动管理命令。浏览器用同样的隔离文件系统和本地 HTTP 服务验证表单、语言切换和移动布局。
